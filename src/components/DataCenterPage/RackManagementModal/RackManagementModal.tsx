...
const RackManagementModal: React.FC<RackManagementModalProps> = ({
    ...
    const handleInternalCellSelect = (
        dcId: number,
        roomId: number,
        rackId: number,
        unit: string
    ...
    const renderRoomHeaders = (room: Room) => {
        const rackData = racks
            ? racks.filter((rack) => room.id === rack.roomId)
            : []
    ...
    const handleSave = () => {
        ...
        if (
            ...
            const areContinuous = sortedUnits.every((unit, index) => {
                ...
                return (
                    parseInt(unit, 10) ===
                    parseInt(sortedUnits[index - 1], 10) + 1
                )
            })
        ...
        <div className={styles.modalOverlay}>
            <div className={styles.modalContent}>
                <h2 className={styles.modalTitle}>
                    編輯機櫃 - {side.charAt(0).toUpperCase() + side.slice(1)}
                </h2>
                <div className={styles.tableWithInputsWrapper}>
                    <div className={styles.inputColumn}>
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                setCurrentRackHeight(e.target.value)
                            }
                        />
                    ...
                    <div className={styles.tableContainer}>
                        <Table
                            key={`rack-management-table-${currentDataCenter.id}`}
                        >
                            <TableHeader>
                                <TableRow>
                                    <TableHead
                                        colSpan={countTotalRacksInDC(
                                ...
                                <TableRow>
                                    ...
                                    ></TableHead>
                                    {filterRoomsByDataCenterId(
                                        currentDataCenter.id
                                    )?.map(renderRoomHeaders)}
                                </TableRow>
                                <TableRow>
