...
const MachineModal: React.FC<MachineModalProps> = ({
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setForm((prev) => ({
            ...prev,
            [name]:
                name === "startUnit" || name === "unit" ? Number(value) : value,
        }))
    }
    ...
    const handleSubmit = async () => {
        ...
        const unitsArray = Array.from(selectedUnits).sort((a, b) => a - b)
        const isContiguous = unitsArray.every(
            (u, i, arr) => i === 0 || u === arr[i - 1] + 1
        )
        ...
        try {
            ...
            if (editmachine) {
                // No need to check payload.id here, it will be in form if editmachine exists
                // Assert that payload.id is a number when calling updateMutation
                await updateMutation.mutateAsync({
                    id: payload.id as number,
                    data: payload as Machine,
                })
            } else {
                    ...
                    <Table className={styles.machineTable}>
                        ...
                        <TableBody>
                            {Array.from({ length: rack.height }, (_, i) => {
                                ...
                                const machineStartUnit = machines.find(
                                    (m) => m.startUnit === unit
                                )
