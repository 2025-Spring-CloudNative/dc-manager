...
export default function ServiceRackMachineRow({
    ...
    const { data: IPaddress, isLoading: isLoadingIPAddress } =
        useGetIPAddressbymachineIdQuery(machine.id!)
    // const { data: IPaddress, isLoading: isLoadingIPAddress} = getlocalIPAddressbyMachineID(machine.id);
    const { data: RoomData, isLoading: isLoadingRoom } = getRoombyid(
        rack.roomId!
    )

    const r = RoomData?.[0]
    const dcId = r?.dataCenterId ?? 0
    const { data: dcData, isLoading: isLoadingDC } =
        useGetDataCenterByIdQuery(dcId)
        ...
        <div className={styles.machineRow}>
            <div
                className={`${styles.status} ${
                    machine.status === "active" ? styles.activeStatus : ""
                }`}
            >
            ...
            <div className={styles.tableRowText} style={{ flex: 1 }}>
                {dcData[0]?.name} / {RoomData[0]?.name} / {rack?.name} / Unit{" "}
                {machine.startUnit}-{machine.startUnit + machine.unit}
            </div>
        </div>
    )
}
